 ...
 * @arg {Snowflake} channel_id - The channel the message was posted in.
 * @arg {String} message - The message to fix.
 */
DCP.fixMessage = function(channel_id, message) {
	var client = this;
	return message.replace(/<@&(\d*)>|<@!(\d*)>|<@(\d*)>|<#(\d*)>/g, function(match, RID, NID, UID, CID) {
		...
		if (RID || NID) {
			if ( client.servers[client.channels[channel_id].guild_id].roles[RID])
				return "@" + client.servers[client.channels[channel_id].guild_id].roles[RID].name;
			if ( client.servers[client.channels[channel_id].guild_id].members[NID]) 
				return "@" + client.servers[client.channels[channel_id].guild_id].members[NID].nick;
		}
	});
};
...
DCP.editServer = function(input, callback) {
	...
	for (var key in input) {
		...
		if (key === 'afk_channel_id') {
			if (server.channels[input[key]] && server.channels[input[key]].type === 2) payload[key] = input[key];
			continue;
		}
		if (key === 'afk_timeout') {
...
DCP.joinVoiceChannel = function(channelID, callback) {
	...
	if (this._vChannels[channelID]) leaveVoiceChannel(this, channelID, callback);
...
function handleWSMessage(data, flags) {
	...
	switch (message.t) {
		...
		case "CHANNEL_CREATE":
			...
			if (!client.servers[_data.guild_id]) return;
...
function joinVoiceChannel(client, voiceSession) {
	if ( !voiceSession.endpoint ) return;
...
function handlevWSClose(voiceSession) {
	...
	if ( voiceSession.emitter ) voiceSession.emitter.emit("disconnect", voiceSession.channelID);
	...
	if ( voiceSession.emitter ) voiceSession.emitter = null
